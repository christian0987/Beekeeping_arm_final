...
SIN_ALPHA_Y = 17 / 480.0   # = 0.05208 pour J3 (vertical)

# ============================================================================
# CONSTANTES POUR L'INSPECTION (AJOUTÉES POUR ÉVITER LES ERREURS)
# ============================================================================
...
def dessiner_resultat(image, pts_cadre, barres_image, roi_memorise, coords_premiere_barre=None, 
    ...
    cv2.putText(result, "b:ANNULER FIGE  d:Debug  c:Coords  a:AlignY  g:AlignAng  v:AlignCenter  s:Sequence  q:Quitter",
...
class BarreDetectionNode(Node):
    ...
    def __init__(self):
        ...
        # Constantes d'inspection (utilisées dans inspect_barre)
        self.HAUTEUR_REMONTEE_INSPECTION = 200.0  # mm - Hauteur de remontée de J2
        self.HAUTEUR_REMONTEE_ROTATION = 50.0     # mm - Hauteur de remontée pour rotation J5
        self.TEMPS_ATTENTE_INSPECTION = 5.0       # secondes
        self.ANGLE_J5_POSITIF = 90.0              # degrés
        self.ANGLE_J5_NEGATIF = -90.0             # degrés
        self.ANGLE_J5_ROTATION = -150.0           # degrés
        self.TOLERANCE_J5 = 10.0                  # degrés
        self.TOLERANCE_J5_ZERO = 0.5              # degrés
        self.TOLERANCE_J4 = 0.5                   # mm
        self.TOLERANCE_J2 = 0.5                   # mm
    ...
    def arm_state_callback(self, msg):
        """
        Callback pour les positions du bras (J0, J1, J2).
        
        Args:
            msg (Float64MultiArray): Positions des joints du bras
        """
        if len(msg.data) >= 3:
    ...
    def wrist_state_callback(self, msg):
        """
        Callback pour les positions du poignet (J3, J4, J5).
        
        Args:
            msg (Float64MultiArray): Positions des joints du poignet
        """
        if len(msg.data) >= 3:
    ...
    def check_positions_received(self):
        """Verifie que toutes les positions sont recues."""
    ...
    def wait_for_joint(self, joint_name, target, tolerance=0.5, timeout=10.0):
        """
        Attend qu'un joint atteigne sa position cible avec affichage continu.
        
        Args:
            joint_name (str): Nom du joint ('j0', 'j1', 'j2', 'j3', 'j4', 'j5')
            target (float): Position cible en mm ou degres
            tolerance (float): Tolerance en mm ou degres
            timeout (float): Timeout en secondes
        
        Returns:
            bool: True si le joint a atteint la cible, False sinon
        """
        start = time.time()
        while time.time() - start < timeout:
            ...
            # AFFICHAGE CONTINU DES IMAGES PENDANT L'ATTENTE
            try:
    ...
    def move_j0_and_wait(self, target, tolerance=0.5, timeout=10.0):
        """Deplace J0 et attend qu'il atteigne la cible."""
    ...
    def move_j1_and_wait(self, target, tolerance=0.5, timeout=10.0):
        """Deplace J1 et attend qu'il atteigne la cible."""
        if self.current_arm_pos is None:
    ...
    def move_j2_and_wait(self, target, tolerance=0.5, timeout=15.0):
        """Deplace J2 et attend qu'il atteigne la cible."""
        if self.current_arm_pos is None:
    ...
    def move_j3_and_wait(self, target, tolerance=0.5, timeout=10.0):
        """Deplace J3 et attend qu'il atteigne la cible."""
    ...
    def move_j4_and_wait(self, target, tolerance=0.5, timeout=10.0):
        """Deplace J4 et attend qu'il atteigne la cible."""
        if self.current_wrist_pos is None:
    ...
    def move_j5_and_wait(self, target, tolerance=0.5, timeout=10.0):
        """Deplace J5 et attend qu'il atteigne la cible."""
        if self.current_wrist_pos is None:
    ...
    # ============================================================
    # Mouvements de base (pour compatibilité avec le code existant)
    # ============================================================
    def move_j0(self, delta_mm):
        """Deplace J0 d'un delta (mouvement relatif)."""
    ...
    def move_j1(self, delta_deg):
        """Deplace J1 d'un delta (mouvement relatif)."""
        if self.current_arm_pos is not None:
            new = self.current_arm_pos.copy()
            new[1] += delta_deg
    ...
    def move_j2(self, delta_mm):
        """Deplace J2 d'un delta (mouvement relatif)."""
        if self.current_arm_pos is not None:
            new = self.current_arm_pos.copy()
            new[2] += delta_mm
    ...
    def move_j4(self, delta_mm):
        """Deplace J4 d'un delta (mouvement relatif)."""
        if self.current_wrist_pos is not None:
            new = self.current_wrist_pos.copy()
            new[1] += delta_mm
    ...
    def move_j5(self, delta_deg):
        """Deplace J5 d'un delta (mouvement relatif)."""
        if self.current_wrist_pos is not None:
            new = self.current_wrist_pos.copy()
            new[2] += delta_deg
    ...
    def wait_for_positions(self, timeout=3.0):
        """Attend que les positions des joints soient recues."""
        start = time.time()
    ...
    def move_j0_to_target(self, target_mm, tolerance=2.0):
        """Deplace J0 a une position cible (avec attente simple)."""
    ...
    # ============================================================
    # ALIGNEMENT VERTICAL (J0)
    # ============================================================
    def align_vertical(self, frame, pts_cadre):
        """Aligne verticalement le cadre avec le centre de la camera (J0)."""
        self.wait_for_positions()
    ...
    def align_angle(self, frame, pts_cadre):
        """Aligne angulairement le cadre avec la camera (J1)."""
        self.wait_for_positions()
        if pts_cadre is None:
    ...
    def align_vertical_centre(self, frame, pts_cadre):
        """Aligne verticalement le centre du cadre avec le centre de la camera (J3)."""
        self.wait_for_positions()
        if pts_cadre is None:
    ...
    # ============================================================
    # ALIGNEMENTS ITERATIFS (J0 <-> J1)
    # ============================================================
    def aligner_complet(self, frame, pts_cadre, max_iterations=10):
        """Aligne J1 et J0 de facon iterative jusqu'a convergence."""
        self.get_logger().info("\n" + "=" * 40)
        self.get_logger().info("ALIGNEMENTS ITERATIFS J1 <-> J0")
        self.get_logger().info("=" * 40)
        
        tolerance_j0 = 6.0  # pixels
        tolerance_j1 = 0.5  # degrés
        
        for iteration in range(max_iterations):
            self.get_logger().info(f"\n--- Iteration {iteration + 1}/{max_iterations} ---")
            
            # 1. Alignement angulaire (J1) - EN PREMIER
            ...
            if pts_cadre is None:
                self.get_logger().warn("⚠️ Cadre perdu pendant l'alignement J1")
                return None
            
            # 2. Alignement vertical (J0) - EN SECOND
            ...
            if pts_cadre is None:
                self.get_logger().warn("⚠️ Cadre perdu pendant l'alignement J0")
                return None
            ...
            if iteration == max_iterations - 1:
                self.get_logger().warn(f"⚠️ Alignements non convergés après {max_iterations} itérations")
    ...
    # ============================================================
    # DESCENTE CALCULEE (sans camera)
    # ============================================================
    def descente_calculee(self):
        """
        Calcule et execute la descente a 2 cm du cadre.
        
        La descente est repartie entre J2 (2/3) et J4 (1/3).
        Des compensations sont appliquees sur J0 et J3.
        """
        ...
        z_cible_mm = 50.0 + self.OFFSET_HAUTEUR_PINCE  # 50 mm + offset vertical
        ...
        # 2. COMPENSATION HORIZONTALE (J0)
        compensation_j0_mm = distance_totale_mm * self.SIN_ALPHA_X
        
        # 3. COMPENSATION VERTICALE (J3)
        ...
        # Mettre à jour j0_centre APRÈS la descente
        if self.current_arm_pos is not None:
    ...
    # ============================================================
    # INSPECTION D'UNE BARRE (AVEC J5 RETOUR A 0 AVANT DESCENTE)
    # ============================================================
    def inspect_barre(self, barre_num):
        """
        Inspection d'une barre : 
        1. VERIFIER que J5 est à 0 (et le remettre si nécessaire)
        2. J2 monte de HAUTEUR_REMONTEE_INSPECTION
        3. J5 tourne à +90°
        4. Attente TEMPS_ATTENTE_INSPECTION
        5. J5 tourne à -90°
        6. Attente TEMPS_ATTENTE_INSPECTION
        7. J5 revient à 0 (CRITIQUE !)
        8. J2 redescend de la MÊME valeur (retour à la position initiale)
        """
        ...
        # Sauvegarder la position J4 actuelle (elle ne doit pas bouger)
        j4_initial = self.current_wrist_pos[1]
        
        # Sauvegarder la hauteur J2 initiale (POUR REDESCENDRE À LA MÊME VALEUR)
        j2_initial = self.current_arm_pos[2]
        
        # ============================================================
        # ÉTAPE 0 : VÉRIFIER QUE J5 EST À 0 AVANT DE DESCENDRE
        # ============================================================
        j5_actuel = self.current_wrist_pos[2]
        if abs(j5_actuel) > self.TOLERANCE_J5_ZERO:
            self.get_logger().warn(f"⚠️ J5 n'est pas à 0 ! Actuel: {j5_actuel:.1f}°")
        ...
        # ============================================================
        # ÉTAPE 1 : Remonter J2 de HAUTEUR_REMONTEE_INSPECTION (AVEC J5 À 0)
        # ============================================================
        ...
        # ============================================================
        # ÉTAPE 3 : Attendre TEMPS_ATTENTE_INSPECTION
        # ============================================================
        ...
        # ============================================================
        # ÉTAPE 5 : Attendre TEMPS_ATTENTE_INSPECTION
        # ============================================================
        ...
        # ============================================================
        # ÉTAPE 7 : Redescendre J2 À LA MÊME HAUTEUR INITIALE (AVEC J5 À 0)
        # ============================================================
        # ✅ CORRECTION : Redescendre de la MÊME valeur pour revenir à j2_initial
        ...
        # ============================================================
        # Vérification finale : J4 n'a pas bougé
        # ============================================================
        if abs(self.current_wrist_pos[1] - j4_initial) > self.TOLERANCE_J4:
            self.get_logger().warn(f"⚠️ J4 a bougé de {self.current_wrist_pos[1] - j4_initial:.1f} mm! Remise à {j4_initial:.1f}")
            self.move_j4_and_wait(j4_initial)
        
        # Vérification finale : J5 est toujours à 0
        if abs(self.current_wrist_pos[2]) > self.TOLERANCE_J5_ZERO:
            self.get_logger().error(f"❌ J5 n'est plus à 0 après descente ! Actuel: {self.current_wrist_pos[2]:.1f}°")
        
        # Vérification finale : J2 est revenu à sa position initiale
        if abs(self.current_arm_pos[2] - j2_initial) > self.TOLERANCE_J2:
            self.get_logger().warn(f"⚠️ J2 n'est pas revenu à sa position initiale ! Actuel: {self.current_arm_pos[2]:.1f} mm, Attendu: {j2_initial:.1f} mm")
    ...
    # ============================================================
    # SEQUENCE COMPLETE + INSPECTION
    # ============================================================
    def sequence_complete(self):
        """
        Execute la sequence complete :
        1. Alignements iteratifs (J1 <-> J0) + J3
        2. Descente à 2 cm (avec compensations J0 et J3)
        3. Navigation vers les barres + inspection (reste à 2 cm)
        4. Retour home (TOUS les joints à 0) AVEC VÉRIFICATION
        """
        self.wait_for_positions()
        ...
        # ============================================================
        # PHASE 1: ALIGNEMENTS ITERATIFS (J1 <-> J0)
        # ============================================================
        self.get_logger().info("\n" + "=" * 40)
        ...
        # 1.1 Alignements iteratifs J1 <-> J0 (commence par J1)
        ...
        # 1.2 Alignement vertical centre (J3) - une seule fois car independant
        self.get_logger().info("\n→ Alignement vertical centre (J3)...")
        _, pts_cadre = self.align_vertical_centre(frame, pts_cadre)
        time.sleep(0.5)
        rclpy.spin_once(self, timeout_sec=0.0)
        
        # ✅ 1.3 REINITIALISER J5 à 0° (APRÈS TOUS LES ALIGNEMENTS)
        self.get_logger().info("\n→ Réinitialisation de J5 à 0° après alignements...")
        if self.current_wrist_pos is not None:
            j5_actuel = self.current_wrist_pos[2]
            if abs(j5_actuel) > self.TOLERANCE_J5_ZERO:
                delta_j5 = 0.0 - j5_actuel
                self.get_logger().info(f"  J5: {j5_actuel:.1f}° -> 0.0° (delta={delta_j5:.1f}°)")
                self.move_j5(delta_j5)  # Mouvement relatif
                time.sleep(0.5)
                rclpy.spin_once(self, timeout_sec=0.0)
                self.get_logger().info(f"✅ J5 réinitialisé à {self.current_wrist_pos[2]:.1f}°")
            else:
                self.get_logger().info("✅ J5 déjà à 0°")
        
        # Enregistrer J0 centre (position ABSOLUE)
        if self.current_arm_pos is not None:
            self.j0_centre = self.current_arm_pos[0]
        ...
        # ============================================================
        # PHASE 2: DESCENTE À 2 CM
        # ============================================================
        self.get_logger().info("\n" + "=" * 40)
        self.get_logger().info("PHASE 2: DESCENTE A 2 CM")
        self.get_logger().info("=" * 40)
        
        # Vérifier que Z a été mesuré
        if self.z_cam_actuel is None:
            self.get_logger().warn("Z non mesuré, tentative de mesure...")
            ret, frame = self.cap.read()
            if ret:
                mask_cadre, _, _, _, _ = masque_cadre(frame)
                pts_cadre, _ = plus_grand_quadrilatere(mask_cadre)
                if pts_cadre is not None and self.barres_figees is not None:
                    M, M_inv = calculer_homographies(pts_cadre)
                    coords = calculer_coordonnees_barre(
                        self.barres_figees[0], pts_cadre, frame.shape, M_inv
                    )
                    self.z_cam_actuel = coords['z_mm']
                    self.get_logger().info(f"Z mesure: {self.z_cam_actuel:.1f} mm")
        
        self.descente_calculee()
        
        # ============================================================
        # PHASE 3: NAVIGATION + INSPECTION DES BARRES
        # ============================================================
        self.get_logger().info("\n" + "=" * 40)
        ...
        for i, barre in enumerate(self.liste_barres):
            ...
            # === TENTATIVE 1 : Avec l'offset actuel ===
            ...
            # Vérifier si la barre est atteignable
            if j0_target < self.arm_limits['j0'][0] or j0_target > self.arm_limits['j0'][1]:
                self.get_logger().warn(f"⚠️ Barre {barre} hors limite: {j0_target:.1f} mm")
                
                # === TENTATIVE 2 : Tourner J5 à -150° (offset = 0) ===
                if not rotation_effectuee:
                    self.get_logger().info("→ Rotation J5 à -150° pour réduire l'offset...")
                    if self.current_wrist_pos is not None:
                        # ✅ 1. REMONTER J2 de HAUTEUR_REMONTEE_ROTATION
                        target_j2 = self.current_arm_pos[2] + self.HAUTEUR_REMONTEE_ROTATION
                        self.get_logger().info(f"  Remontée J2 de {self.HAUTEUR_REMONTEE_ROTATION:.1f} mm: {self.current_arm_pos[2]:.1f} -> {target_j2:.1f} mm")
                        self.move_j2_and_wait(target_j2)
                        time.sleep(0.5)
                        rclpy.spin_once(self, timeout_sec=0.0)
                        
                        # ✅ 2. Tourner J5 vers -150° (RELATIF)
                        target_j5 = self.ANGLE_J5_ROTATION
                        delta_j5 = target_j5 - self.current_wrist_pos[2]
                        self.get_logger().info(f"  J5: {self.current_wrist_pos[2]:.1f}° -> {target_j5:.1f}° (delta={delta_j5:.1f}°)")
                        self.move_j5(delta_j5)  # Mouvement relatif
                        time.sleep(2.0)
                        rclpy.spin_once(self, timeout_sec=0.0)
                        
                        # ✅ 3. VÉRIFICATION AVEC TOLÉRANCE
                        if self.current_wrist_pos is not None:
                            j5_actuel = self.current_wrist_pos[2]
                            
                            if abs(j5_actuel - target_j5) <= self.TOLERANCE_J5:
                                # Cas 1 : J5 est proche de la cible
                                self.get_logger().info(f"✅ J5 à {j5_actuel:.1f}° (cible: {target_j5}°, OK)")
                                self.OFFSET_PINCE = 0.0
                                rotation_effectuee = True
                                self.get_logger().info(f"✅ OFFSET_PINCE = {self.OFFSET_PINCE:.1f} mm")
                            else:
                                # Cas 2 : J5 n'a pas atteint la cible mais on continue
                                self.get_logger().warn(f"⚠️ J5 à {j5_actuel:.1f}° (cible: {target_j5}°, écart: {abs(j5_actuel - target_j5):.1f}°)")
                                self.get_logger().info("→ On continue avec la position atteinte")
                                
                                # Calcul d'un offset partiel
                                ...
                                rotation_effectuee = True
                                self.get_logger().info(f"✅ OFFSET_PINCE ajusté : {self.OFFSET_PINCE:.1f} mm")
                        
                        # ✅ 4. Redescendre J2
                        target_j2 = self.current_arm_pos[2] - self.HAUTEUR_REMONTEE_ROTATION
                        self.get_logger().info(f"  Redescente J2: {self.current_arm_pos[2]:.1f} -> {target_j2:.1f} mm")
                        self.move_j2_and_wait(target_j2)
                        time.sleep(0.5)
                        rclpy.spin_once(self, timeout_sec=0.0)
                        
                        # ✅ 5. Recalculer la cible
                        j0_target = self.j0_centre + self.barres_offsets[barre] + self.OFFSET_PINCE
                        self.get_logger().info(f"  Nouvelle cible J0: {j0_target:.1f} mm")
                        
                        # Vérifier si la barre est maintenant atteignable
                        if j0_target < self.arm_limits['j0'][0] or j0_target > self.arm_limits['j0'][1]:
                            self.get_logger().warn(f"⚠️ Barre {barre} toujours hors limite: {j0_target:.1f} mm")
                            self.get_logger().info(f"→ Barre {barre} ignorée, passage à la suivante")
                            barres_ignorees.append(barre)
                            continue
                        else:
                            self.get_logger().info(f"✅ Barre {barre} maintenant atteignable: {j0_target:.1f} mm")
                    else:
                        self.get_logger().warn("⚠️ Position J5 inconnue, barre ignorée")
                        barres_ignorees.append(barre)
                        continue
                else:
                    # Rotation déjà effectuée
                    self.get_logger().warn(f"⚠️ Barre {barre} toujours hors limite après rotation J5")
                    self.get_logger().info(f"→ Barre {barre} ignorée, passage à la suivante")
                    barres_ignorees.append(barre)
                    continue
            ...
            # Pause entre les barres
            time.sleep(0.5)
            rclpy.spin_once(self, timeout_sec=0.0)
        
        # === RAPPORT DES BARRES IGNORÉES ===
        if barres_ignorees:
            self.get_logger().warn(f"\n⚠️ Barres ignorées (hors limite): {barres_ignorees}")
        else:
        ...
        # ============================================================
        # PHASE 4: RETOUR HOME (TOUS LES JOINTS À 0) AVEC VÉRIFICATION
        # ============================================================
        self.get_logger().info("\n" + "=" * 40)
        self.get_logger().info("PHASE 4: RETOUR HOME")
        self.get_logger().info("=" * 40)
        
        # ✅ APPEL DE go_home() QUI RETOURNE LES POSITIONS
        ...
        if tous_a_zero:
            ...
            self.get_logger().warn("⚠️ Des joints ne sont pas à 0 - Vérifier le retour home")
            self.get_logger().warn("   Positions finales :")
            for nom, valeur in positions_finales.items():
                if valeur is not None:
                    self.get_logger().warn(f"     {nom}: {valeur:.1f}")
    ...
    def go_home(self):
        """
        Retourne a la position home : TOUS les joints a 0.
        Avec vérification et retour des valeurs.
        """
        self.get_logger().info("=" * 60)
        ...
        # ✅ DICTIONNAIRE POUR STOCKER LES POSITIONS FINALES
        positions_finales = {
            'j0': None,
            'j1': None,
            'j2': None,
        ...
        # Ordre : J2 d'abord (pour éviter de toucher), puis J5, puis les autres
        # J2 -> 0
        if self.current_arm_pos is not None:
            self.get_logger().info("1. J2 -> 0")
            success = self.move_j2_and_wait(0.0)
            time.sleep(0.5)
            rclpy.spin_once(self, timeout_sec=0.0)
            if not success:
        ...
        if self.current_wrist_pos is not None:
            ...
            success = self.move_j5_and_wait(0.0)
            time.sleep(0.5)
            rclpy.spin_once(self, timeout_sec=0.0)
            if not success:
        ...
        success = self.move_j0_and_wait(0.0)
        time.sleep(0.5)
        rclpy.spin_once(self, timeout_sec=0.0)
        if not success:
        ...
        if self.current_arm_pos is not None:
            ...
            success = self.move_j1_and_wait(0.0)
            time.sleep(0.5)
            rclpy.spin_once(self, timeout_sec=0.0)
            if not success:
        ...
        if self.current_wrist_pos is not None:
            ...
            success = self.move_j3_and_wait(0.0)
            time.sleep(0.5)
            rclpy.spin_once(self, timeout_sec=0.0)
            if not success:
        ...
        if self.current_wrist_pos is not None:
            ...
            success = self.move_j4_and_wait(0.0)
            time.sleep(0.5)
            rclpy.spin_once(self, timeout_sec=0.0)
            if not success:
                erreurs.append("J4 n'a pas atteint 0")
        
        # ✅ VÉRIFICATION FINALE AVEC RETOUR DES VALEURS
        self.get_logger().info("\n" + "=" * 60)
        self.get_logger().info("=== VÉRIFICATION FINALE DES POSITIONS ===")
        self.get_logger().info("=" * 60)
        
        # Attendre que les positions soient à jour
        time.sleep(0.5)
        rclpy.spin_once(self, timeout_sec=0.0)
        ...
        if self.current_arm_pos is not None and self.current_wrist_pos is not None:
            ...
            erreur_totale = (abs(positions_finales['j0']) + 
                            abs(positions_finales['j1']) + 
                            abs(positions_finales['j2']) + 
                            abs(positions_finales['j3']) + 
                            abs(positions_finales['j4']) + 
                            abs(positions_finales['j5']))
            
            self.get_logger().info(f"\n📈 Erreur totale cumulée : {erreur_totale:.1f}")
            self.get_logger().info(f"📊 Nombre de joints hors tolérance : {sum(1 for v in positions_finales.values() if v is not None and abs(v) >= 0.5)}")
            
            # ✅ VÉRIFICATION QUE TOUS LES JOINTS SONT BIEN À 0
            tous_a_zero = all(abs(v) < 0.5 for v in positions_finales.values() if v is not None)
            
            if tous_a_zero:
                self.get_logger().info("\n" + "🎉" * 15)
                self.get_logger().info("✅ TOUS LES JOINTS SONT À 0 !")
                self.get_logger().info("🎉" * 15)
            else:
                self.get_logger().warn("\n" + "⚠️" * 15)
                self.get_logger().warn("⚠️ ATTENTION : Certains joints ne sont pas à 0 !")
                self.get_logger().warn("⚠️" * 15)
                
                # ✅ TENTATIVE DE CORRECTION POUR LES JOINTS HORS TOLÉRANCE
                self.get_logger().info("\n→ Tentative de correction...")
                for nom, valeur in positions_finales.items():
                    if valeur is not None and abs(valeur) >= 0.5:
                        self.get_logger().info(f"  Correction de {nom} ({valeur:.1f} -> 0.0)")
                        if nom == 'j0':
                ...
                if self.current_arm_pos is not None and self.current_wrist_pos is not None:
                    ...
                    self.get_logger().info("=" * 40)
                    self.get_logger().info(f"  J0 : {self.current_arm_pos[0]:8.1f} mm  {'✅' if abs(self.current_arm_pos[0]) < 0.5 else '❌'}")
                    self.get_logger().info(f"  J1 : {self.current_arm_pos[1]:8.1f} deg {'✅' if abs(self.current_arm_pos[1]) < 0.5 else '❌'}")
                    self.get_logger().info(f"  J2 : {self.current_arm_pos[2]:8.1f} mm  {'✅' if abs(self.current_arm_pos[2]) < 0.5 else '❌'}")
                    self.get_logger().info(f"  J3 : {self.current_wrist_pos[0]:8.1f} mm  {'✅' if abs(self.current_wrist_pos[0]) < 0.5 else '❌'}")
                    self.get_logger().info(f"  J4 : {self.current_wrist_pos[1]:8.1f} mm  {'✅' if abs(self.current_wrist_pos[1]) < 0.5 else '❌'}")
                    self.get_logger().info(f"  J5 : {self.current_wrist_pos[2]:8.1f} deg {'✅' if abs(self.current_wrist_pos[2]) < 0.5 else '❌'}")
                    self.get_logger().info("=" * 40)
                    
                    # Mettre à jour tous_a_zero après correction
                    positions_finales['j0'] = self.current_arm_pos[0]
                    positions_finales['j1'] = self.current_arm_pos[1]
                    positions_finales['j2'] = self.current_arm_pos[2]
                    positions_finales['j3'] = self.current_wrist_pos[0]
        ...
        # ✅ RETOURNER LES POSITIONS POUR UNE ÉVENTUELLE UTILISATION EXTERNE
    ...
    def run(self):
        ...
        Gere la capture video, la detection et l'interface utilisateur.
        """
...
def main(args=None):
    ...
    Args:
        args: Arguments de ligne de commande (non utilises)
    """
