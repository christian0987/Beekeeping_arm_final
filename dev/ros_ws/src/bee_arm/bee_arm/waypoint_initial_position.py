...
class GoToHomeSafe(Node):
    ...
    def __init__(self):
        ...
        # Joint limits (sécurité) - AVEC MARGE
        self.arm_limits = {
            'j0': (-325.0, 325.0),
            'j1': (-45.0, 180.0),   
            'j2': (-430.0, 370.0),
        ...
        # ==============================================
        # DÉFINITION DES POSITIONS PRÉDÉFINIES
        # ==============================================
        # Format: (arm [j0, j1, j2], wrist [j3, j4, j5])
        # Unités: j0, j2, j3, j4 en mm | j1, j5 en degrés
        
        self.positions = {
            'home': (
                [0.0, 0.0, 0.0],      # arm: j0=0mm, j1=0°, j2=0mm
                [0.0, 0.0, 0.0]       # wrist: j3=0mm, j4=0mm, j5=0°
            ),
            
            'standby': (
                [0.0, 179.0, 0.0],   # arm: j0=0mm, j1=179°, j2=0mm
                [0.0, 0.0, 0.0]       # wrist: j3=0mm, j4=0mm, j5=0°
            ),

        }
        
        # Tolérance pour les vérifications
        self.limit_tolerance = 0.5
        
        # Paramètres de mouvement
        self.publish_rate = 30.0      # Hz - plus élevé pour plus de fluidité
        
        self.get_logger().info("GoToHomeSafe node started")
        self.get_logger().info("Available positions: home, standby")
    ...
    def check_limits(self, arm_target, wrist_target):
        ...
        if not (self.arm_limits['j0'][0] - tol <= arm_target[0] <= self.arm_limits['j0'][1] + tol):
            self.get_logger().warn(f"J0 target {arm_target[0]:.2f} mm corrigé")
        ...
        if not (self.arm_limits['j1'][0] - tol <= arm_target[1] <= self.arm_limits['j1'][1] + tol):
            self.get_logger().warn(f"J1 target {arm_target[1]:.2f} deg corrigé")
        ...
        if not (self.arm_limits['j2'][0] - tol <= arm_target[2] <= self.arm_limits['j2'][1] + tol):
            self.get_logger().warn(f"J2 target {arm_target[2]:.2f} mm corrigé")
        ...
        if not (self.wrist_limits['j3'][0] - tol <= wrist_target[0] <= self.wrist_limits['j3'][1] + tol):
            self.get_logger().warn(f"J3 target {wrist_target[0]:.2f} mm corrigé")
        ...
        if not (self.wrist_limits['j4'][0] - tol <= wrist_target[1] <= self.wrist_limits['j4'][1] + tol):
            self.get_logger().warn(f"J4 target {wrist_target[1]:.2f} mm corrigé")
        ...
        if not (self.wrist_limits['j5'][0] - tol <= wrist_target[2] <= self.wrist_limits['j5'][1] + tol):
            self.get_logger().warn(f"J5 target {wrist_target[2]:.2f} deg corrigé")
    ...
    def get_accelerated_fraction(self, t, accel_percent):
        ...
        # Si on est dans la phase d'accélération
        if t <= accel_percent:
            # Accélération progressive (courbe quadratique)
            normalized_t = t / accel_percent
            # Accélération douce puis linéaire
            return normalized_t * normalized_t * 0.5
        
        # Phase de vitesse constante
        else:
            # Valeur à la fin de l'accélération (50% de la vitesse max)
            accel_end_value = 0.5
            # Progression linéaire après l'accélération
    ...
    def _move_to_target(self, end_arm, end_wrist, duration, position_name=""):
        ...
        # 15% du mouvement pour l'accélération
        accel_percent = 0.15
        
        self.get_logger().info(f"Moving to {position_name} in {duration}s")
...
def main(args=None):
    ...
    try:
        # ==============================================
        # CHOISISSEZ LA POSITION DÉSIRÉE ICI
        # ==============================================
        
        # Aller à la position home (COMMENTÉ - DÉSACTIVÉ)
        node.go_to_home(duration=30.0)  # 30 secondes
        
        # Aller à la position standby (ACTIF) - VOTRE POSITION MODIFIÉE
        #node.go_to_standby(duration=25.0)  # 25 secondes
        
        # ==============================================
        # POUR AJUSTER LA VITESSE :
        # - Durée plus longue = plus lent
        # - Durée plus courte = plus rapide
        # 
        # Exemples :
        # node.go_to_standby(duration=35.0)  # Très lent
        # node.go_to_standby(duration=25.0)  # Moyen (recommandé)
        # node.go_to_standby(duration=15.0)  # Rapide
        # ==============================================
